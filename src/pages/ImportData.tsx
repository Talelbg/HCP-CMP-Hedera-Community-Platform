...
const ImportData: React.FC = () => {
  ...
  const [importSummary, setImportSummary] = useState<{ processed: number, failed: number } | null>(null);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        setPreviewData(result.validRecords);
  ...
  const handleImport = async () => {
    ...
    setIsUploading(true);
    try {
      const results = await subscriptionService.bulkAddSubscriptions(previewData);
      const failed = results.filter((r: any) => r.status === 'failed').length;
      const processed = results.length - failed;

      setImportSummary({ processed, failed });
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-8">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Bulk upload participant data using CSV.
          </p>
        </header>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white dark:bg-dark-panel p-6 rounded-lg shadow border border-gray-200 dark:border-gray-800">
              ...
              <div className="flex items-center justify-center w-full">
                <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 dark:border-gray-700 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
            ...
            {importStatus === 'success' && importSummary && (
                <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-800">
                    ...
                    <p className="text-sm text-green-600 dark:text-green-300">
                        Successfully processed {importSummary.processed} records.
